  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        const rawError = err.response?.data?.error ?? err.message ?? "Failed to scan wallet address.";
        // Guard against non-string error payloads - e.g. a platform-level crash
        // (like Vercel's own function error format) returns `error` as an OBJECT
        // ({ code, message }), not a string. Rendering that directly in JSX
        // throws "Objects are not valid as a React child" and blanks the page.
        const errorMsg =
          typeof rawError === "string"
            ? rawError
            : rawError?.message || "Something went wrong on the server. Please try again.";
        setError(errorMsg);
